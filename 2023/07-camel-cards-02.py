...
def evaluateHandTypes(hands: List[List[str]]) -> None:
    for handNum in range(len(hands)):
        ...
        if JOKER not in differentCards:
            ...
            for card in CARDS:
                if card == JOKER:
                    continue
                newHand = hands[handNum][0].replace(JOKER, card)
                maxType = max(maxType, setHandType(newHand, list(set(newHand))))
                if maxType == 7:
                    break
            hands[handNum][2] = maxType
