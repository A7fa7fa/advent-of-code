...
def extrapolateEnd(data: List[List[int]]) -> int:
    lastExtrapolatedValue = data[-1][-1]
    for i in range(len(data)-2, -1, -1):
        lastExtrapolatedValue = data[i][-1] + lastExtrapolatedValue
...
for history in reduce(histories):
    result += extrapolateEnd(history)

print("result : " + str(result)) # 1974913025
